...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100,view_num=0):
        super().__init__()
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            view_index = (torch.randint(high=self.view_num,size=(1,)) % self.view_num)/self.view_num # [0, 1)
            thetas = torch.FloatTensor([self.opt.default_polar]).to(self.device)
            phis = torch.FloatTensor([view_index * 360]).to(self.device)
            radius = torch.FloatTensor([self.opt.default_radius]).to(self.device)
            poses, dirs = circle_poses(self.device, radius=radius, theta=thetas, phi=phis, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front)
            # fixed focal
            fov = self.opt.default_fovy
            ...
            # circle pose
            view_index = (index[0]%self.size)/self.size # [0, 1)
            thetas = torch.FloatTensor([self.opt.default_polar]).to(self.device)
            phis = torch.FloatTensor([view_index * 360]).to(self.device)
            radius = torch.FloatTensor([self.opt.default_radius]).to(self.device)
            poses, dirs = circle_poses(self.device, radius=radius, theta=thetas, phi=phis, return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front)
