...
def normal_consistency(face_normals, t_pos_idx):
    ...
    # n0 and n1 are two normals for two face, and these two faces share one edge
    # if n0 and n1 parallel -> the model is smooth if totally smooth, loss = 0  or the loss increase
    # Compute error metric based on normal difference
    term = torch.clamp(torch.sum(n0 * n1, -1, keepdim=True), min=-1.0, max=1.0)
    term = (1.0 - term) # hope face to be smooth so that n0 * n1 = 1
...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.opt.dmtet:
            ...
            self.verts = - torch.tensor(tets['vertices'], dtype=torch.float32, device='cuda') * 2 # covers [-1, 1] 277410
    ...
    def run_cuda(self, rays_o, rays_d, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, perturb=False, T_thresh=1e-4, binarize=False, tv_reg=False, **kwargs):
        # rays_o, rays_d: [B, N, 3]
        # return: image: [B, N, 3], depth: [B, N]
        # perturb False when eval   True when train means whether sample the point along the ray uniformly
        # binary False when eval and train
        prefix = rays_o.shape[:-1]
        rays_o = rays_o.contiguous().view(-1, 3)
        rays_d = rays_d.contiguous().view(-1, 3)
...
# dmtet init
        if mesh is not None: #F
            ...
            self.fix_sdf = self.sdf.clone().detach()
    ...
    def run_dmtet(self, rays_o, rays_d, mvp, h, w, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=None, **kwargs):
        ...
        # self.verts: all the grid
        # verts: valid grid
        # faces: verts index for all valid faces
        # get normals
        ...
        normal, _ = dr.interpolate(vn.unsqueeze(0).contiguous(), rast, faces) #[B, H*W, 3]
        ...
        albedo = torch.zeros_like(xyzs, dtype=torch.float32) # [H*W,3]
        if mask.any():
            # dmtet render only here use net
            masked_albedo = self.density(xyzs[mask])['albedo']
            albedo[mask] = masked_albedo.float()
        albedo = albedo.view(-1, h, w, 3) # [H,W]
        ...
        if shading == 'albedo':
            ...
            lambertian = ambient_ratio + (1 - ambient_ratio)  * (normal * light_d).sum(-1).float().clamp(min=0)
            color = albedo * lambertian.unsqueeze(-1) #[H, W]
        ...
        if self.opt.lambda_2d_normal_smooth > 0 or self.opt.lambda_normal > 0: #F
...
# Render
    ...
    def render(self, rays_o, rays_d, mvp, h, w, staged=False, max_ray_batch=4096, **kwargs):
        ...
        # Nerf staege: stage True when eval  False when train, but both no use
        B, N = rays_o.shape[:2]
        device = rays_o.device
