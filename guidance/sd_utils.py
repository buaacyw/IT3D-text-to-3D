...
class StableDiffusion(nn.Module):
    def __init__(self, opt,device, fp16, vram_O,gan=False, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        super().__init__()
        self.opt = opt
        self.device = device
        ...
        # Create model
        if gan:
            self.pipe_img2img = StableDiffusionImg2ImgPipeline.from_pretrained(model_key, torch_dtype=self.precision_t)
            self.pipe_img2img.set_progress_bar_config(disable=True)

            self.pipe_img2img.to(device)
            self.vae = self.pipe_img2img.vae
        ...
        if self.opt.lora_dir is not None:
            self.unet.load_attn_procs(os.path.join(self.opt.lora_dir,"unet.safetensors"), use_safetensors=True)
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, grad_scale=1,
        ...
        # w(t), sigma_t^2
        w = (1 - self.alphas[t])
        diff = w[:, None, None, None] * (noise_pred - noise) # latent
    ...
    def train_step_perpneg(self, text_embeddings, weights, pred_rgb, guidance_scale=100, as_latent=False, grad_scale=1,
        ...
        if as_latent:
            ...
            pred_rgb_512 = F.interpolate(pred_rgb, (512, 512), mode='bilinear', align_corners=False)
            # encode image into latents with vae, requires grad!
            latents = self.encode_imgs(pred_rgb_512) # with randomness but tiny
        ...
        w = (1 - self.alphas[t]) # B, 4, 64. 64 latent
        diff = w[:, None, None, None] * (noise_pred - noise)
        grad = grad_scale * diff
        grad = torch.nan_to_num(grad)
