...
def depth_gen(rendering_list, embeding_list, opt):
    ...
    iterator = tqdm(rendering_list, desc='CTN Depth', total=len(rendering_list))
    ...
    for view_index, input_image in enumerate(iterator):
        ...
        input_image = (input_image * 255).clip(0,255).astype(np.uint8) # 0, 255
        res=[]
